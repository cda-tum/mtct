...
void cda_rail::solver::mip_based::VSSGenTimetableSolver::
  ...
  m_vars["x_in"]    = MultiArray<GRBVar>(num_tr, num_t);
  m_vars["len_out"] = MultiArray<GRBVar>(num_tr, num_t);
  m_vars["x_out"]   = MultiArray<GRBVar>(num_tr, num_t);
  m_vars["e_lda"]   = MultiArray<GRBVar>(num_tr, num_t, num_edges);
  m_vars["e_mu"]    = MultiArray<GRBVar>(num_tr, num_t, num_edges);
...
void cda_rail::solver::mip_based::VSSGenTimetableSolver::
  ...
  for (size_t tr = 0; tr < num_tr; ++tr) {
    ...
    for (size_t t = train_interval[tr].first; t <= train_interval[tr].second;
      ...
      // v(t+1))/2 * dt + brakelen (if applicable)
      GRBLinExpr lhs = m_vars["len_in"](tr, t) + m_vars["len_out"](tr, t);
      for (size_t e = 0; e < num_edges; ++e) {
        lhs += m_vars["e_mu"](tr, t, e) - m_vars["e_lda"](tr, t, e);
      }
      GRBLinExpr rhs =
      ...
      for (size_t v = 0; v < num_vertices; ++v) {
        const auto out_edges = m_instance.get_editable_network().out_edges(v);
        const auto in_edges  = m_instance.get_editable_network().in_edges(v);
        lhs                  = m_vars["x_v"](tr, t, v);
        GRBLinExpr rhs_in    = 0;
        GRBLinExpr rhs_out   = 0;
        for (const auto& e : out_edges) {
          rhs_out += m_vars["x"](tr, t, e);
        }
        for (const auto& e : in_edges) {
          rhs_in += m_vars["x"](tr, t, e);
        }
        if (v == exit) {
          rhs_out += m_vars["x_out"](tr, t);
        }
        if (v == entry) {
          rhs_in += m_vars["x_in"](tr, t);
        }
        m_model->addConstr(lhs, GRB_LESS_EQUAL, rhs_out + rhs_in,
                           "train_pos_x_v_" + tr_name + "_" +
                               std::to_string(t) + "_" + std::to_string(v));
        m_model->addConstr(lhs, GRB_GREATER_EQUAL, rhs_out,
                           "train_pos_x_v_out_" + tr_name + "_" +
                               std::to_string(t) + "_" + std::to_string(v));
        m_model->addConstr(lhs, GRB_GREATER_EQUAL, rhs_in,
                           "train_pos_x_v_in_" + tr_name + "_" +
                               std::to_string(t) + "_" + std::to_string(v));
      }
      ...
      for (size_t v = 0; v < num_vertices; ++v) {
        rhs += m_vars["x_v"](tr, t, v);
      }
      m_model->addConstr(lhs, GRB_EQUAL, rhs,
                         "train_pos_simple_connected_path_" + tr_name + "_" +
                             std::to_string(t));

      // Switches are obeyed, i.e., illegal movements prohibited
      // And train does not go backwards
      for (size_t e1 = 0; e1 < num_edges; ++e1) {
        const auto& v = m_instance.get_editable_network().get_edge(e1).target;
        const auto& out_edges = m_instance.get_editable_network().out_edges(v);
        const auto& e_len =
            m_instance.get_editable_network().get_edge(e1).length;
        for (const auto& e2 : out_edges) {
          if (t < train_interval[tr].second &&
              m_instance.get_editable_network().is_valid_successor(e1, e2)) {
            // Prohibit train going backwards
            // x_e1(t+1) <= x_e1(t) + (1-x_e2(t))
            m_model->addConstr(
                m_vars["x"](tr, t + 1, e1), GRB_LESS_EQUAL,
                m_vars["x"](tr, t, e1) + (1 - m_vars["x"](tr, t, e2)),
                "train_pos_no_backwards_" + tr_name + "_" + std::to_string(t) +
                    "_" + std::to_string(e1) + "_" + std::to_string(e2));
          } else if (!m_instance.get_editable_network().is_valid_successor(
                         e1, e2)) {
            // Prohibit illegal movement
            // x_e1 + x_e2 <= 1
            m_model->addConstr(
                m_vars["x"](tr, t, e1) + m_vars["x"](tr, t, e2), GRB_LESS_EQUAL,
                1,
                "train_pos_switches_" + tr_name + "_" + std::to_string(t) +
                    "_" + std::to_string(e1) + "_" + std::to_string(e2));
          }
        }
        ...
        if (t < train_interval[tr].second) {
          ...
          m_model->addConstr(m_vars["e_lda"](tr, t, e1), GRB_LESS_EQUAL,
                             m_vars["e_lda"](tr, t + 1, e1) +
                                 e_len * (1 - m_vars["x"](tr, t + 1, e1)),
                             "train_pos_e_lda_" + tr_name + "_" +
                                 std::to_string(t) + "_" + std::to_string(e1));
          m_model->addConstr(m_vars["e_mu"](tr, t, e1), GRB_LESS_EQUAL,
                             m_vars["e_mu"](tr, t + 1, e1) +
                                 e_len * (1 - m_vars["x"](tr, t + 1, e1)),
                             "train_pos_e_mu_" + tr_name + "_" +
                                 std::to_string(t) + "_" + std::to_string(e1));
        }
      }
      if (t < train_interval[tr].second) {
        ...
        m_model->addConstr(m_vars["len_in"](tr, t + 1), GRB_LESS_EQUAL,
                           m_vars["len_in"](tr, t),
                           "train_pos_len_in_" + tr_name + "_" +
                               std::to_string(t));
        m_model->addConstr(m_vars["len_out"](tr, t + 1), GRB_GREATER_EQUAL,
                           m_vars["len_out"](tr, t),
                           "train_pos_len_out_" + tr_name + "_" +
                               std::to_string(t));
      }
    }
  }
...
void cda_rail::solver::mip_based::VSSGenTimetableSolver::
  ...
  for (size_t tr = 0; tr < num_tr; ++tr) {
    ...
    for (size_t t = train_interval[tr].first;
      ...
      m_model->addConstr(lhs, GRB_EQUAL, rhs,
                         "train_pos_overlap_len_" + tr_name + "_" +
                             std::to_string(t));

      // Determine overlap value per edge
      for (size_t e = 0; e < num_edges; ++e) {
        const auto& e_v0 = m_instance.get_editable_network().get_edge(e).source;
        const auto& e_v1 = m_instance.get_editable_network().get_edge(e).target;
        const auto& out_edges =
            m_instance.get_editable_network().out_edges(e_v1);
        ...
        m_model->addConstr(m_vars["overlap"](tr, t, e) +
                               e_len * (1 - m_vars["x"](tr, t + 1, e)),
                           GRB_GREATER_EQUAL,
                           m_vars["e_mu"](tr, t, e) -
                               m_vars["e_lda"](tr, t + 1, e),
                           "train_pos_overlap_e_lb_" + tr_name + "_" +
                               std::to_string(t) + "_" + std::to_string(e));
        // overlap <= e_mu(t) - e_lda(t+1)
        m_model->addConstr(m_vars["overlap"](tr, t, e), GRB_LESS_EQUAL,
                           m_vars["e_mu"](tr, t, e) -
                               m_vars["e_lda"](tr, t + 1, e),
                           "train_pos_overlap_e_ub_" + tr_name + "_" +
                               std::to_string(t) + "_" + std::to_string(e));

        // overlap <= e_len * x_e(t)
        // overlap <= e_len * x_e(t+1)
        m_model->addConstr(m_vars["overlap"](tr, t, e), GRB_LESS_EQUAL,
                           e_len * m_vars["x"](tr, t, e),
                           "train_pos_overlap_e_t_" + tr_name + "_" +
                               std::to_string(t) + "_" + std::to_string(e));
        m_model->addConstr(m_vars["overlap"](tr, t, e), GRB_LESS_EQUAL,
                           e_len * m_vars["x"](tr, t + 1, e),
                           "train_pos_overlap_e_tp1_" + tr_name + "_" +
                               std::to_string(t) + "_" + std::to_string(e));

        // Overlap is only at front
        for (const auto& e2 : out_edges) {
          if (m_instance.get_editable_network().is_valid_successor(e, e2)) {
            // overlap_e <= e_len * overlap_e2 + e_len * (1 - x_e2)
            m_model->addConstr(m_vars["overlap"](tr, t, e), GRB_LESS_EQUAL,
                               e_len * m_vars["overlap"](tr, t, e2) +
                                   e_len * (1 - m_vars["x"](tr, t, e2)),
                               "train_pos_overlap_at_front_" + tr_name + "_" +
                                   std::to_string(t) + "_" + std::to_string(e) +
                                   "_" + std::to_string(e2));
          }
        }
        if (e_v0 == entry) {
          ...
          m_model->addConstr(m_vars["len_in"](tr, t), GRB_LESS_EQUAL,
                             tr_len * m_vars["overlap"](tr, t, e) +
                                 tr_len * (1 - m_vars["x"](tr, t, e)),
                             "train_pos_overlap_at_front_" + tr_name + "_" +
                                 std::to_string(t) + "_len_in" +
                                 std::to_string(e));
        }
        if (e_v1 == exit) {
          // overlap_e <= e_len * len_out + e_len * (1 - x_out)
          m_model->addConstr(m_vars["overlap"](tr, t, e), GRB_LESS_EQUAL,
                             e_len * m_vars["len_out"](tr, t) +
                                 e_len * (1 - m_vars["x_out"](tr, t)),
                             "train_pos_overlap_at_front_" + tr_name + "_" +
                                 std::to_string(t) + "_len_out" +
                                 std::to_string(e));
        }
      }
    }
...
void cda_rail::solver::mip_based::VSSGenTimetableSolver::
  ...
  for (size_t tr = 0; tr < num_tr; ++tr) {
    ...
    // len_in(t0) = tr_len
    m_model->addConstr(m_vars["len_in"](tr, t0), GRB_EQUAL, tr_len,
                       "train_boundary_len_in_" + tr_name + "_" +
                           std::to_string(t0));
    // len_out(tn) = tr_len + brakelen(tn) (if applicable)
    GRBLinExpr rhs = tr_len;
    if (this->include_braking_curves) {
      rhs += m_vars["brakelen"](tr, tn);
    }
    m_model->addConstr(m_vars["len_out"](tr, tn), GRB_EQUAL, rhs,
                       "train_boundary_len_out_" + tr_name + "_" +
                           std::to_string(tn));
  }
}
...
void cda_rail::solver::mip_based::VSSGenTimetableSolver::
  ...
  for (size_t tr = 0; tr < num_tr; ++tr) {
    ...
    for (size_t e = 0; e < num_edges; ++e) {
      const auto& e_v0 = m_instance.get_editable_network().get_edge(e).source;
      const auto& e_v1 = m_instance.get_editable_network().get_edge(e).target;
      const auto& in_edges  = m_instance.get_editable_network().in_edges(e_v0);
      const auto& out_edges = m_instance.get_editable_network().out_edges(e_v1);
      const auto  e_len = m_instance.get_editable_network().get_edge(e).length;
      for (size_t t = train_interval[tr].first; t <= train_interval[tr].second;
           ++t) {
        // e_lda <= e_mu
        m_model->addConstr(
            m_vars["e_lda"](tr, t, e), GRB_LESS_EQUAL, m_vars["e_mu"](tr, t, e),
            "train_occupation_free_routes_mu_lda_" + tr_name + "_" +
                std::to_string(t) + "_" + std::to_string(e));
        // e_mu <= e_len * x
        m_model->addConstr(m_vars["e_mu"](tr, t, e), GRB_LESS_EQUAL,
                           e_len * m_vars["x"](tr, t, e),
                           "train_occupation_free_routes_mu_x_" + tr_name +
                               "_" + std::to_string(t) + "_" +
                               std::to_string(e));
        ...
        GRBLinExpr rhs = 0;
        for (const auto& e2 : out_edges) {
          rhs += m_vars["x"](tr, t, e2);
        }
        if (e_v1 == exit) {
          // exit is an out-edge of the last edge
          rhs += m_vars["x_out"](tr, t);
        }
        rhs *= e_len;
        m_model->addConstr(
            m_vars["e_mu"](tr, t, e) + e_len * (1 - m_vars["x"](tr, t, e)),
            GRB_GREATER_EQUAL, rhs,
            "train_occupation_free_routes_mu_1_if_not_last_edge_" + tr_name +
                "_" + std::to_string(t) + "_" + std::to_string(e));

        // e_lda = 0 if not first edge, i.e.,
        // e_lda <= e_len * (1 - sum_inedges x) + e_len * (1-x)
        rhs = 2 - m_vars["x"](tr, t, e);
        for (const auto& e2 : in_edges) {
          rhs -= m_vars["x"](tr, t, e2);
        }
        if (e_v0 == entry) {
          // entry is an in-edge of the first edge
          rhs -= m_vars["x_in"](tr, t);
        }
        rhs *= e_len;
        m_model->addConstr(
            m_vars["e_lda"](tr, t, e), GRB_LESS_EQUAL, rhs,
    ...
    for (size_t t = train_interval[tr].first; t <= train_interval[tr].second;
      ...
      m_model->addConstr(m_vars["x_in"](tr, t), GRB_LESS_EQUAL,
                         m_vars["len_in"](tr, t),
                         "train_occupation_free_routes_x_in_1_only_if_" +
                             tr_name + "_" + std::to_string(t));
      m_model->addConstr(tr_len * m_vars["x_in"](tr, t), GRB_GREATER_EQUAL,
                         m_vars["len_in"](tr, t),
                         "train_occupation_free_routes_x_in_1_if_" + tr_name +
                             "_" + std::to_string(t));

      // x_out = 1 if, and only if, len_out > 0, i.e.,
      // x_out <= len_out, len_out_ub * x_out >= len_out
      m_model->addConstr(m_vars["x_out"](tr, t), GRB_LESS_EQUAL,
                         m_vars["len_out"](tr, t),
                         "train_occupation_free_routes_x_out_1_only_if_" +
                             tr_name + "_" + std::to_string(t));
      m_model->addConstr(len_out_ub * m_vars["x_out"](tr, t), GRB_GREATER_EQUAL,
                         m_vars["len_out"](tr, t),
                         "train_occupation_free_routes_x_out_1_if_" + tr_name +
                             "_" + std::to_string(t));
    }
  }
}
...
void cda_rail::solver::mip_based::VSSGenTimetableSolver::
  ...
  for (size_t tr = 0; tr < train_list.size(); ++tr) {
    ...
    for (size_t t = train_interval[tr].first; t <= train_interval[tr].second;
      ...
      const auto dist_travelled_before = max_distance_travelled(
          tr, t_steps_before, before_after_struct.v_before,
      ...
      const auto dist_travelled_after = max_distance_travelled(
          tr, t_steps_after, before_after_struct.v_after,
      ...
      for (size_t e = 0; e < num_edges; ++e) {
        ...
        if (before_after_struct.t_before <= train_interval[tr].first) {
          auto entry_edges = m_instance.get_editable_network().out_edges(
              m_instance.get_const_schedule(tr).get_entry_vertex());
          if (entry_edges.empty()) {
            dist_before = INF;
          } else {
            const auto e_before = *entry_edges.begin();
            const auto e_len_before =
                m_instance.get_editable_network().get_edge(e_before).length;
        ...
        if (dist_travelled_before < dist_before) {
          ...
          m_model->addConstr(
              m_vars["x"](tr, t, e), GRB_EQUAL, 0,
              "train_occupation_free_routes_impossibility_before_var1_" +
                  tr_name + "_" + std::to_string(t) + "_" + std::to_string(e));
          ...
          m_model->addConstr(
              m_vars["e_mu"](tr, t, e), GRB_LESS_EQUAL,
              dist_travelled_before - dist_before,
              "train_occupation_free_routes_impossibility_before_var2_" +
                  tr_name + "_" + std::to_string(t) + "_" + std::to_string(e));
        }
        ...
        if (before_after_struct.t_after >= train_interval[tr].second) {
          auto exit_edges = m_instance.get_editable_network().in_edges(
              m_instance.get_const_schedule(tr).get_exit_vertex());
          if (exit_edges.empty()) {
            dist_after = INF;
          } else {
            const auto e_after = *exit_edges.begin();
          ...
          for (const auto& e_tmp : before_after_struct.edges_after) {
            const auto tmp_val =
                apsp[e][e_tmp] -
                m_instance.get_editable_network().get_edge(e_tmp).length;
        ...
        if (dist_travelled_after < dist_after) {
          ...
          m_model->addConstr(
              m_vars["x"](tr, t, e), GRB_EQUAL, 0,
              "train_occupation_free_routes_impossibility_after_var1_" +
                  tr_name + "_" + std::to_string(t) + "_" + std::to_string(e));
          ...
          m_model->addConstr(
              m_vars["e_lda"](tr, t, e), GRB_GREATER_EQUAL,
              (e_len - (dist_travelled_after - dist_after)) *
                  m_vars["x"](tr, t, e),
              "train_occupation_free_routes_impossibility_after_var2_" +
                  tr_name + "_" + std::to_string(t) + "_" + std::to_string(e));
        }
      }
    }
      ...
      for (size_t t = train_interval[tr].first; t <= train_interval[tr].second;
        ...
        for (size_t vss = 0; vss < vss_number_e; ++vss) {
          ...
          m_model->addConstr(
              ...
              "train_occupation_free_routes_vss_lda_b_pos_b_front_" + tr_name +
          ...
          if (m_instance.get_const_train_list().get_train(tr).has_tim()) {
            const auto m2 = e_len;
            m_model->addConstr(
                m_vars["b_pos"](e_index, vss), GRB_LESS_EQUAL,
                m_vars["e_lda"](tr, t, e) +
  ...
  for (const auto& tr_list : train_entry_exit_pairs.first) {
    for (size_t i = 0; i < tr_list.size() - 1; ++i) {
      ...
      for (size_t t = tr2_entry; t < train_interval[tr_list[i]].second; ++t) {
        ...
        m_model->addConstr(m_vars["len_in"](tr_list[i], t), GRB_EQUAL, 0,
                           "train_occupation_free_routes_common_entry_len_in_" +
                               std::to_string(tr_list[i]) + "_" +
                               std::to_string(tr_list[i + 1]) + "_" +
                               std::to_string(t));
        m_model->addConstr(m_vars["x_in"](tr_list[i], t), GRB_EQUAL, 0,
                           "train_occupation_free_routes_common_entry_x_in_" +
                               std::to_string(tr_list[i]) + "_" +
                               std::to_string(tr_list[i + 1]) + "_" +
                               std::to_string(t));
      }
  ...
  for (const auto& tr_list : train_entry_exit_pairs.second) {
    for (size_t i = 0; i < tr_list.size() - 1; ++i) {
      ...
      for (size_t t = train_interval[tr_list[i]].first; t <= tr2_exit; ++t) {
        ...
        m_model->addConstr(m_vars["len_out"](tr_list[i], t), GRB_EQUAL, 0,
                           "train_occupation_free_routes_common_exit_len_out_" +
                               std::to_string(tr_list[i]) + "_" +
                               std::to_string(tr_list[i + 1]) + "_" +
                               std::to_string(t));
        m_model->addConstr(m_vars["x_out"](tr_list[i], t), GRB_EQUAL, 0,
                           "train_occupation_free_routes_common_exit_x_out_" +
                               std::to_string(tr_list[i]) + "_" +
                               std::to_string(tr_list[i + 1]) + "_" +
                               std::to_string(t));
      }
