...
cda_rail::solver::mip_based::VSSGenTimetableSolver::initialize_variables(
  ...
  if (!instance.get_editable_network().is_consistent_for_transformation()) {
  ...
  num_edges    = instance.get_editable_network().number_of_edges();
  num_vertices = instance.get_editable_network().number_of_vertices();

  unbreakable_sections = instance.get_editable_network().unbreakable_sections();
...
cda_rail::solver::mip_based::VSSGenTimetableSolver::optimize(
  ...
  for (const auto& e : relevant_edges) {
    obj_ub += instance.get_const_network().max_vss_on_edge(e);
  }
  double obj_lb           = 0;
