...
TEST(GeneralPerformanceOptimizationInsancens, FixGenPO) {
  std::filesystem::path const wd           = "data";
  std::string const           subdirectory = "gen-po";
  // Expect directory data\instances\atmos2023 to exist
  std::filesystem::path const dir_path = wd / "instances" / subdirectory;
      ...
      for (size_t v = 0; v < instance.get_const_network().number_of_vertices();
        ...
        for (auto const& entry : entry_information.at(v)) {
          ...
          if (entry.entry_time < earliest_entry_time) {
            ...
            tr_schedule.set_entry_time(tr_schedule.get_entry_time() +
                                       time_diff);
          ...
          earliest_entry_time =
              tr_schedule.get_entry_time() + vertex_obj.headway;
        }
      }
