...
template <typename GeneType, typename MiddleCostType> struct ChromosomeType {
  GeneType       genes;
...
template <typename GeneType, typename MiddleCostType> struct GenerationType {
...
template <typename GeneType, typename MiddleCostType> class Genetic {
private:
...
public:
  ...
  Genetic()
      : unif_dist(0.0, 1.0), N_robj(0), problem_mode(GA_MODE::SOGA),
        ...
        tol_stall_best(1e-6), best_stall_max(10), reference_vector_divisions(0),
        enable_reference_vectors(true), multi_threading(true),
        dynamic_threading(true), N_threads(std::thread::hardware_concurrency()),
  ...
  void solve_init() {
    ...
    if (!is_single_objective()) {
      ...
      if (!reference_vector_divisions) {
        ...
        if (N_robj == 1)
          throw runtime_error("The length of objective vector is 1 in a "
                              "multi-objective optimization");
        while (get_number_reference_vectors(N_robj, reference_vector_divisions +
                                                        1) <= (int)population)
...
protected:
  ...
  double random01() {
    std::lock_guard<std::mutex> lock(
        mtx_rand); // prevent data race between threads
  ...
  void check_settings() {
    if (is_interactive()) {
      ...
      if (distribution_objective_reductions != nullptr)
        throw runtime_error("distribution_objective_reductions is not null in "
                            "interactive mode!");
      ...
      if (eval_solution != nullptr)
        throw runtime_error("eval_solution is not null in interactive mode "
                            "(use eval_solution_IGA instead)!");
    } else {
      if (calculate_IGA_total_fitness != nullptr)
        throw runtime_error(
      ...
      if (is_single_objective()) {
        ...
        if (distribution_objective_reductions != nullptr)
          throw runtime_error("distribution_objective_reductions is not null "
    ...
    if (is_single_objective()) { // SO (including IGA)
      if (SO_report_generation == nullptr)
        throw runtime_error("SO_report_generation is not adjusted while "
                            "problem mode is single-objective");
      if (MO_report_generation != nullptr)
        throw runtime_error("MO_report_generation is adjusted while problem "
                            "mode is single-objective");
    } else {
      if (SO_report_generation != nullptr)
        throw runtime_error("SO_report_generation is adjusted while problem "
                            "mode is multi-objective");
      if (MO_report_generation == nullptr)
        throw runtime_error("MO_report_generation is not adjusted while "
  ...
  template <typename T> unsigned int index_of_min(const vector<T>& v) {
    ...
    if (use_quick_sort) {
      ...
      std::sort(gen.sorted_indices.begin(), gen.sorted_indices.end(),
                [&gen](int a, int b) -> bool {
                  return gen.chromosomes[a].total_cost <
  ...
  bool init_population_try(thisGenerationType& generation0,
                           thisChromosomeType& X, int index) {
    if (is_interactive()) {
  ...
  void init_population_range(thisGenerationType* p_generation0, int index_from,
                             int index_to, unsigned int* attemps,
  ...
  template <void (thisType::*action_function)(
      thisGenerationType* p_generation0, int index_from, int index_to,
      unsigned int* attemps, std::atomic<bool>& active_thread)>
  void sequential_action(thisGenerationType& generation, unsigned int N_add,
  ...
  template <void (thisType::*action_function)(
      thisGenerationType* p_generation0, int index_from, int index_to,
      unsigned int* attemps, std::atomic<bool>& active_thread)>
  void dynamic_thread_action(thisGenerationType& generation, unsigned int N_add,
                             unsigned int& total_attempts) {
    ...
    while (x_index < N_add && !user_request_stop) {
      ...
      if (free_thread > -1) {
        ...
        thread_pool[free_thread]    = std::thread(
            action_function, this, &generation,
            offset + int(x_index), /* from */
            offset + int(x_index), /* to */
            &attempts[free_thread], std::ref(active_threads[free_thread]));
        x_index++;
      } else {
        idle();
      }
  ...
  template <void (thisType::*action_function)(
      thisGenerationType* p_generation0, int index_from, int index_to,
      unsigned int* attemps, std::atomic<bool>& active_thread)>
  void static_thread_action(thisGenerationType& generation, unsigned int N_add,
                            unsigned int& total_attempts) {
    vector<atomic<bool>> active_threads(N_threads);
    for (auto& at : active_threads)
      std::atomic_init(&at, false);
    ...
    for (int i = 0; i < N_threads; i++) {
      if (i + 1 == N_threads) // last chunk
        ...
        x_index_end = std::min(x_index_start + pop_chunk - 1,
                               int(generation.chromosomes.size()) - 1);

      if (x_index_end >= x_index_start) {
        active_threads[i] = true;
        thread_pool[i] =
            std::thread(action_function, this, &generation, x_index_start,
                        x_index_end, &attempts[i], std::ref(active_threads[i]));
      }
  ...
  void init_population(thisGenerationType& generation0) {
    ...
    generation0.chromosomes.reserve(
        population); // push_back can invalidate the vector

    unsigned int new_solutions_offset =
        (unsigned int)generation0.chromosomes.size();
    unsigned int N_add =
        (unsigned int)std::max(0, int(population) - int(new_solutions_offset));
    ...
    if (!multi_threading || N_threads == 1 || is_interactive()) {
      sequential_action<&thisType::init_population_range>(generation0, N_add,
                                                          total_attempts);
    } else {
    ...
    for (int index = x_index_begin; index <= x_index_end; index++) {
      ...
      while (!successful) {
        ...
        if (random01() <= mutation_rate) {
          ...
          X.genes =
              mutate(X.genes, [this]() { return random01(); }, shrink_scale);
        }
        if (is_interactive()) {
  ...
  void crossover_and_mutation(thisGenerationType& new_generation) {
    ...
    if (is_interactive()) {
      if (N_add + elite_count != population)
        throw runtime_error("In IGA mode, elite fraction + crossover fraction "
    ...
    if (verbose) {
      cout << "Mutations and crossovers of " << N_add
           << " solutions are calculated with " << total_attempts << " attemps."
           << endl;
    }
  }
